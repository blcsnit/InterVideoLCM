...
class CodeFormerJointModel(SRModel):
    ...
    def optimize_parameters(self, current_iter):
        ...
        embed_stacked = torch.empty(1,0,768).to(self.device)


        #4f = 
        for i in range(self.video_frame):
            input_std = self.preprocess(self.input[:, i])
            embed = self.clip_model.encode_image(input_std).to(torch.float)
            embed_stacked = torch.cat((embed_stacked, embed), dim = 1)

        visual_feat = self.visual_encoder(embed_stacked)  # output of Visual Encoder

        torch.cuda.empty_cache()

        latent_code = self.alcm.vae.encode(self.input[0])['latent_dist'].mean
        
        latent_code = latent_code * 0.18215
        self.output = lcm_pipecall.call(self.alcm,
                                        latent_code,
                                        prompt="A woman with brown stright hair is speaking.",
                                        negative_prompt="",
                                        num_frames=self.video_frame,
                                        guidance_scale=1.0,
                                        num_inference_steps=self.num_inference_steps,
                                        generator=torch.Generator("cpu").manual_seed(0),
                                        input_embed = visual_feat,
                                        lq_input=self.input[0]
                                        ).to(self.device)
        ...
        if current_iter % self.net_d_iters == 0:  #and current_iter > self.net_g_start_iter:
            ...
            if not large_de:  # when large degradation don't need image-level loss
                if self.cri_pix:
                    l_g_pix = self.cri_pix(torch.transpose(self.output[0], 0, 1), self.gt[0])
                    l_g_total += l_g_pix
                    loss_dict['l_g_pix'] = l_g_pix

                # perceptual loss
                if self.cri_perceptual:
                    l_g_percep = self.cri_perceptual(torch.transpose(self.output[0], 0, 1), self.gt[0])
                    l_g_total += l_g_percep
                    loss_dict['l_g_percep'] = l_g_percep

                # gan loss
                if  current_iter > self.net_d_start_iter:
                    fake_g_pred = self.net_d(self.output[0].permute(1, 0, 2, 3))
        ...
        if not large_de:
            if current_iter > self.net_d_start_iter:
                ...
                # real
                real_d_pred = self.net_d(self.gt.detach()[0])
                l_d_real = self.cri_gan(real_d_pred, True, is_disc=True)
                loss_dict['l_d_real'] = l_d_real
                loss_dict['out_d_real'] = torch.mean(real_d_pred.detach())
                l_d_real.backward()
                # fake
                fake_d_pred = self.net_d(self.output.detach()[0].permute(1, 0, 2, 3))
