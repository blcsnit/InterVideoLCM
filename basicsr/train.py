...
def save_images_as_gif(image_tensor, output_path, duration=0.1):
    images_np = image_tensor.permute(0, 2, 3, 1).cpu().numpy()

    if images_np.dtype != np.uint8:
        if np.max(images_np) <= 1.0 + 1e-6 and np.min(images_np) >= -1e-6:
            images_np = (images_np * 255).astype(np.uint8)
        else:
            images_np = ((images_np - np.min(images_np)) / (np.max(images_np) - np.min(images_np)) * 255).astype(np.uint8)
    
    iio.imwrite(output_path, images_np, duration=duration)

def train_pipeline(root_path):
    # parse options, set distributed setting, set random seed
    opt = parse_options(root_path, is_train=True)
    video_frame = opt["lcm"]["video_frame"]
    opt['visual_encoder']['video_frame'] = video_frame
    opt['datasets']['train']['video_frame'] = video_frame
        ...
        while train_data is not None:
            ...
            if train_data['stat'] == True:
                if opt['rank'] == 0 and current_iter % opt['logger']['print_freq'] == 0 or current_iter == 1:
                    ...
                    with torch.no_grad():
                        # RE
                        video1 = model.alcm.video_processor.postprocess_video(video=model.output, output_type="pil")
                        export_to_gif(video1[0], f'{save_path}/{iter}iter_lcm_output_.gif')
                        # LQ
                        video2 = model.alcm.video_processor.postprocess_video(video=model.input.transpose(1, 2), output_type="pil")
                        export_to_gif(video2[0], f'{save_path}/{iter}iter_lq.gif')
                        # GT
                        video3 = model.alcm.video_processor.postprocess_video(video=model.gt.transpose(1, 2), output_type="pil")
                        export_to_gif(video3[0], f'{save_path}/{iter}iter_gt_iter.gif')
